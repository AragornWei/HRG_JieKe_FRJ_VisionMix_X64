...
namespace Yoga.Common
{
    ...
    public static class Util
    {
        ...
        private static ManualResetEvent notifyRunSignal = new ManualResetEvent(true);
        private static ManualResetEvent notifyErrRunSignal = new ManualResetEvent(true);
        /// <summary>
        /// 输出日志到Log4Net
        /// </summary>
        /// <param name="t"></param>
        ...
        public static void Notify(string message)
        {
            ...
            {
                if(notifyRunSignal.WaitOne(1000))
                {
        ...
        public static void Notify(Level level, string message)
        {
            if (MessageEvent != null)
            {
                if (notifyErrRunSignal.WaitOne(1000))
                {
                    MessageEvent(null, new MessageEventArgs(level, message));
                }            
            }
            notifyErrRunSignal.Set();
        }
        /// <summary>
