...
namespace Yoga.Common.TCPHelper
{
    public class TCPServerHelper
    {
        ...
        /// <param name="encodingMathord">通讯的编码方式</param> 
        ...
        public virtual void Start()
        {
            ...
            //设置异步方法接受客户端连接---连接状态信息包含在Socket对象：_svrSock里面。
            _svrSock.BeginAccept(new AsyncCallback(AcceptConn), _svrSock);//第二个参数表示An object that contains state information for this request.。
        ...
        protected virtual void AcceptConn(IAsyncResult iar)
        {
            ...
            // AsyncState：Get the socket that handles the client request.
        ...
        protected virtual void ReceiveData(IAsyncResult iar)
        {
            ...
            try
            {
                ...
                _receivedData = _coder.GetEncodingString(_recvDataBuffer, 0, recv);//通过BeginReceive()函数注册，已由系统将接收数据放入_recvDataBuffer。
                //发布收到数据的事件
                if (RecvData != null)
                {
                    Session sendDataSession = FindSession(client);
                    Debug.Assert(sendDataSession != null);
                    ICloneable copySession = sendDataSession; 
                    Session clientSession = (Session)copySession.Clone();//调用了Session类中的object ICloneable.Clone()函数。所以克隆结果要强制类型转换
                    clientSession.Datagram = _coder.GetTextBytes(_receivedData);  //Array.Copy(_recvDataBuffer, 0, clientSession.Datagram, 0, recv);也可以
                    RecvData(this, new NetEventArgs(clientSession));//把数据及发送方的IP地址等到放在一个Session对象里面发送过来。这个Session一定要克隆过来。
                }
                //end of if(RecvData!=null)
                //继续接收来自来客户端的数据--相当于将底层发送数据方的发送数据运行信号量激活。
