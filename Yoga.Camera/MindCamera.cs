...
namespace Yoga.Camera
{
    ...
    public class MindCamera : CameraBase
    {
        ...
        protected CAMERA_CONNECTION_STATUS_CALLBACK m_CameraConnectionStatusCallback;
            ...
            else if (Phase == 2)
            {
                ...
                try
                {
                    ...
                    //{
                        ...
                        {
                            Util.Notify(Common.Basic.Level.Err, string.Format("相机{0}编码格式不正确,当前格式{1}", cameraIndex, pFrameHead.uiMediaType));
                        }
                        //hPylonImage.MirrorImage("row");     //由open函数中的MvApi.CameraSetMirror(m_hCamera, 1, 1)执行，省去一直执行。            
                        TrigerImageEvent();
                    //}
                }
        ...
        private void CameraConnectionStatusCallback(CameraHandle hCamera, uint MSG, uint uParam, IntPtr pContext)  //MSG:0相机断开，1相机恢复。
        {
            if (MSG == 0)
            {
                TrigerCameraLineOff();
                //Util.Notify(Common.Basic.Level.Err, $"相机{cameraIndex}断线请检查");
                //ContinuousShotStop();
                //IsLink = false;
                ////Close();                
            }                
        ...
        public override void OneShot(Command command)
        {
            try
            {
                ...
                Command = command;
                CameraSdkStatus status;
                //status = MvApi.CameraSetTriggerMode(m_hCamera, (int)emSdkSnapMode.SOFT_TRIGGER);  //已经在ContinuousShotStop()中设置过。
        ...
        public override void SetExtTrigger()
        {
            try
            {
                CameraSdkStatus st = MvApi.CameraSetTriggerMode(m_hCamera, (int)emSdkSnapMode.EXTERNAL_TRIGGER);
                st= MvApi.CameraSetExtTrigSignalType(m_hCamera, 1);//下降沿触发             
