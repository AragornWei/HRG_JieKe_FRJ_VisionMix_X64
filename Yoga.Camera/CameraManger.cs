                ...
                if ((CameraFlag & CameraFlag.Basler) == CameraFlag.Basler)
                {
                    ...
                    if (g_allCameras.Count > 0)
                    {
                        for (int i = 0; i < g_allCameras.Count; i++)
                        {
                            ...
                            if (CameraDic.Keys.Contains(i + 1) == false)
                            {
                                CameraDic.Add(i + 1, new BaslerCamera(g_camera, i + 1));
                                cameraCount++;
                            }
                            else
                            {
                            ...
                            //    {
                            //        CameraDic.Add(id1, new BaslerCamera(g_camera, i));
                            //        cameraCount++;
                            //    }
                            //}
                    ...
                    if (0 == nRet && m_pDeviceList.nDeviceNum > 0)
                    {
                        ...
                        (MyCamera.MV_CC_DEVICE_INFO)Marshal.PtrToStructure(m_pDeviceList.pDeviceInfo[i],
                            ...
                            if (device.nTLayerType == MyCamera.MV_GIGE_DEVICE)
                            {
                                ...
                                int key;
                                string gigeIp = gigeInfo.nCurrentIp.ToString();
                                ...
                                if (CameraDic.Keys.Contains(i + 1) == false)
                                {
                                    ...
                                    {
                                        CameraDic.Add(i + 1, new HikvisionCamera(m_pOperator, i + 1));
                                        cameraCount++;
                                    }
                                }
                                else
                            ...
                            for (int i = 0; i < tCameraDevInfoList.Count(); i++)
                            {
                                ...
                                if (status == CameraSdkStatus.CAMERA_STATUS_SUCCESS)
                                {
                                    ...
                                    else
                                    {
                                        //Util.Notify("相机ID未设置");
                                        string message;
                                        if (CameraDic.Keys.Contains(key))
                                        {
                                            message = string.Format("相机{0}名称设置与前面相机重复", i + 1);
                                            MessageBox.Show(message, "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        }
                                        else
                                        {
                                            message = string.Format("相机{0}名称设置错误", i + 1);
                                            MessageBox.Show(message, "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                        }
