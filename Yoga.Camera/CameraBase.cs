...
{    
    public abstract class CameraBase
    {
       ...
       public event EventHandler<EventArgs> CameraLineOffEvent;

        #region 相机参数
        protected int cameraIndex;
        protected int settingIndex;
        protected bool isGrabberManual;
        ...
        protected ManualResetEvent imageResetEvent = new ManualResetEvent(true);
       ...
       /// </summary>
        ...
        protected void TrigerCameraLineOff()
        {
            if (CameraLineOffEvent != null)
                CameraLineOffEvent(this, null);
        }
        ...
        public virtual string GetCameraAcqInfo()
        {
            ...
            if (IsContinuousShot)
            {
                info = string.Format("FPS:{0:F1}|帧:{1}|", fps.GetFps(), fps.GetTotalFrameCount());
            }
            else
            {
                info = string.Format(" 帧:{0} |", fps.GetTotalFrameCount());
            }
            return info;
        }
        /// <summary>
        ...
        {
            isGrabberManual = true;
            imageResetEvent.Reset();
            OneShot(Command.Grab);
            if (imageResetEvent.WaitOne(delayMs)&& hPylonImage.IsInitialized())     //WaitOne()阻塞线程。
            {
                isGrabberManual = false;
                return hPylonImage;
            }
            else
            {
                isGrabberManual = false;
                Util.Notify(Common.Basic.Level.Err, "图像软触发采集超时");
                return null;
                //throw new Exception("图像软触发采集超时");
            }
        }

        public virtual void GrabImage(int delayMs,int settingIndex)
        {
            isGrabberManual = false;
            if (imageResetEvent.WaitOne(delayMs))     //WaitOne()阻塞线程。
            {
