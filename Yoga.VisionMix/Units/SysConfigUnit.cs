...
namespace Yoga.VisionMix.Units
{
    public partial class SysConfigUnit : UserControl
    {
        ...
        private void Btn_SaveCOM1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                UserSetting.Instance.MainDeviceComParam.Use = chkUseIO.Checked;
                if (chkUseIO.Checked)
                {
                    UserSetting.Instance.MainDeviceComParam.ComName = drpComList1.Text;
                    UserSetting.Instance.MainDeviceComParam.BaudRate = drpBaudRate1.Text;

                    UserSetting.Instance.MainDeviceComParam.Parity = drpParity1.SelectedIndex.ToString();
                    UserSetting.Instance.MainDeviceComParam.DataBits = drpDataBits1.Text;
                    UserSetting.Instance.MainDeviceComParam.StopBits = drpStopBits1.Text;
                }
                UserSetting.Instance.SaveSetting();
                Camera.IOSerial.Instance.Rs232Param = UserSetting.Instance.MainDeviceComParam;
