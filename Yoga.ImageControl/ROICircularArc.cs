		...
		public override void MoveByHandle(double newX, double newY)
		{
			...
			switch (activeHandleIdx)
			{
				...
				case 2: // start handle for arc                
					...
					if (extentPhi < 0 && prior > PI * 0.8)//逆时针；旋转超0.8*π
						extentPhi = (PI + extentPhi) + PI;//结果旋转大于180度,旋转方向逆时针。
					else if (extentPhi > 0 && prior < -PI * 0.7)//顺时针旋转超0.7*π
				...
				case 3: // end handle for arc
					...
					if (circDir == "positive" && startPhi >= next)//逆时针旋180-extentPhi
						extentPhi = (next + TwoPI) - startPhi;//逆时针旋>180
                    else if (circDir == "positive" && next > startPhi)
						extentPhi = next - startPhi;//逆时针旋<180
                    else if (circDir == "negative" && startPhi >= next)
						extentPhi = -1.0 * (startPhi - next);//顺时针旋<180
                    else if (circDir == "negative" && next > startPhi)
						extentPhi = -1.0 * (startPhi + TwoPI - next);//顺时针旋>180
